...
for feature = 1 : length(fftEx)
    ...
    for gesture = 1:length(gestures)
        rawData = readtable(strcat(num2str(user),'_',char(gestures(gesture))));
        L = width(rawData) - 1;
        t = (0:L-1)*T;        
        f = Fs*(0:L/2)/L;
        Y = 0;    
    ...
    featmat1=featmat1(1:datapoint-1,:);
    offset = offset + 3;
end

% DWT
featmat2 = zeros(200,12);
offset1 = 1;
datapoint1 = 1;
dwtEx = [12,15,19,11];
for feature = 1:length(dwtEx)
    ...
    featmat2=featmat2(1:datapoint1-1,:);
    offset1 = offset1 + 3;
end

%rms
rmsEx = [23,24,25,26,27];
featmat3 = zeros(200,5);
datapoint2 = 1;
for feature = 1:length(rmsEx)
    ...
    featmat3=featmat3(1:datapoint2-1,:);
end

%variance
varianceEx = [24,25,26,27,28,23,22,15,21,14];
featmat4 = zeros(200,10);
datapoint3 = 1;
for feature = 1:length(varianceEx)
    ...
    featmat4=featmat4(1:datapoint3-1,:);
end
pnewf = [11,7,8,18,12,20,14,9,16,10];
featmat5 = zeros(200,10);
datapoint = 1
for feature = 1:length(pnewf)
    ...
    featmat5=featmat5(1:datapoint-1,:);
...
%final2=final2(1:datapoint-1,:);
csvwrite(strcat(num2str(user),"_","featurematrix.csv"),final2);
end
